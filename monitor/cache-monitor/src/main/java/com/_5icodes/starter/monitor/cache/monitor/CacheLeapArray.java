...
import com._5icodes.starter.common.utils.AssertUtil;
 ...
 */
public abstract class CacheLeapArray<T> {
    ...
    protected final AtomicReferenceArray<CacheWindowWrap<T>> array;
     ...
     * @param intervalInMs the total time interval of this {@link CacheLeapArray} in milliseconds
     */
    public CacheLeapArray(int sampleCount, int intervalInMs) {
     ...
     */
    public CacheWindowWrap<T> currentWindow() {
     ...
     */
    protected abstract CacheWindowWrap<T> resetWindowTo(CacheWindowWrap<T> windowWrap, long startTime);
     ...
     */
    public CacheWindowWrap<T> currentWindow(long timeMillis) {
        if (timeMillis < 0) {
            return null;
        }
         ...
         */
        while (true) {
            CacheWindowWrap<T> old = array.get(idx);
            if (old == null) {
                /*
                 *     B0       B1      B2    NULL      B4
                 ...
                 */
                CacheWindowWrap<T> window = new CacheWindowWrap<T>(windowLengthInMs, windowStart, newEmptyBucket(timeMillis));
                ...
                return new CacheWindowWrap<T>(windowLengthInMs, windowStart, newEmptyBucket(timeMillis));
            }
        }
    }
     ...
     */
    public CacheWindowWrap<T> getPreviousWindow(long timeMillis) {
        if (timeMillis < 0) {
            return null;
        }
        int idx = calculateTimeIdx(timeMillis - windowLengthInMs);
        timeMillis = timeMillis - windowLengthInMs;
        CacheWindowWrap<T> wrap = array.get(idx);
     ...
     */
    public CacheWindowWrap<T> getPreviousWindow() {
    ...
    public T getWindowValue(long timeMillis) {
        ...
        CacheWindowWrap<T> bucket = array.get(idx);
     ...
     */
    public boolean isWindowDeprecated(/*@NonNull*/ CacheWindowWrap<T> windowWrap) {
        return isWindowDeprecated(TimeUtil.currentTimeMillis(), windowWrap);
    }

    public boolean isWindowDeprecated(long time, CacheWindowWrap<T> windowWrap) {
     ...
     */
    public List<CacheWindowWrap<T>> list() {
        return list(TimeUtil.currentTimeMillis());
    }

    public List<CacheWindowWrap<T>> list(long validTime) {
        int size = array.length();
        List<CacheWindowWrap<T>> result = new ArrayList<>(size);

        for (int i = 0; i < size; i++) {
            CacheWindowWrap<T> windowWrap = array.get(i);
     ...
     */
    public List<CacheWindowWrap<T>> listAll() {
        int size = array.length();
        List<CacheWindowWrap<T>> result = new ArrayList<>(size);
     ...
     */
    CacheWindowWrap<T> getValidHead(long timeMillis) {
     ...
     * @return the "head" bucket if it exists and is valid; otherwise null
     */
    public CacheWindowWrap<T> getValidHead() {
    ...
    public void debug(long time) {
        ...
        List<CacheWindowWrap<T>> lists = list(time);
        sb.append("Thread_").append(Thread.currentThread().getId()).append("_");
        for (CacheWindowWrap<T> window : lists) {
